...
export type Phase = "bidding" | "selectTrump" | "playing" | "gameOver";
...
export function legalMoves(state: GameState): Move[] {
  ...
  if (state.phase === "selectTrump") {
    ...
    return SUITS.map((suit) => ({ type: "selectTrump", seat, suit }));
  }
...
export function applyMove(state: GameState, move: Move): GameState {
  ...
  if (state.phase === "selectTrump") {
    if (move.type !== "selectTrump") throw new Error("expected selectTrump");
    return {
      ...state,
      trump: move.suit,
      phase: "playing",
      turn: state.winningBid!.seat,
      leaderSeat: state.winningBid!.seat,
    };
  }
  if (state.phase === "playing") return applyPlay(state, move);
...
function applyBid(state: GameState, move: Move): GameState {
  ...
  // Bidding resolved. highBid is guaranteed non-null (dealer is forced).
  return {
    ...state,
    highBid,
    bidsActed,
