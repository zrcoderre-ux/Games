// GENERATED by 'npm run build' from src/client-local.ts — do not edit by hand.
...
var LocalRoom = class {
  ...
  start(config) {
    ...
    this.config = config;
    this.seats = seats;
    const seed = config.seed !== void 0 ? config.seed : (Date.now() ^ Math.random() * 4294967295) >>> 0;
...
function unseenTrumpCount(state, trump, myCards) {
  ...
  const seenInTricks = state.tricksWon.flatMap((t) => t.plays).filter((p) => isTrump(p.card, trump)).length + state.currentTrick.filter((p) => isTrump(p.card, trump)).length;
...
function higherTrumpsAllAccountedFor(state, trump, card, myCards) {
  const cardVal = trumpValue(card, trump);
  const low = lowRankFor(state.players);
  const playedOrOwned = /* @__PURE__ */ new Set();
  for (const t of state.tricksWon) {
...
function tricksRemaining(state, seat) {
  const handSize3 = state.hands[seat].length + state.tricksWon.length + (state.currentTrick.length > 0 ? 1 : 0);
